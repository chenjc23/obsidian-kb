...
3. 写在哪按体量决定(避免过度拆文件):简单分支就地写在 `主干流程.md` 对应步骤下;够分量的(逻辑链长、深嵌套、高风险、业务关键、被复用)提取独立分支页——先 `scaffold flow --repo {repo} --topic {主题} --member 分支主题` 拿骨架,按实际分支主题命名 Write(一个分支一份)。
