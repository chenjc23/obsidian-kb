...
5. **视图不必平均分配目录**：按该视图"持久手写内容"多少加权。影响视图大部分**靠工具自动生成**，只留少量人工叙事页。
...
code-kb/
  ...
  use-cases/                    # 用例视图(+1):跨仓场景目录 = agent 主入口
    {用例名}.md                 #   (只新增,不改旧)
  domains/                      # 逻辑视图:业务域
    _map.md                     #   业务域地图                        (自动生成,勿手写)
    {业务域}.md                 #   (只新增,不改旧)
  contracts/                    # 契约视图:跨边界契约
    _map.md                     #   契约地图                          (自动生成,勿手写)
    {契约名}.md                 #   (只新增,不改旧)
  architecture/                 # 实现视图·工作区汇总
    system-architecture.md      #   (人工叙事,改动只标 stale)
    dependency-graph.md         #   从 depends-on/双链自动生成        (自动生成,勿手写)
    tech-stack.md               #   从各 repo architecture/manifest 汇 (自动生成,勿手写)
    shared-patterns.md          #   跨仓真有复现模式才建              (人工叙事,改动只标 stale)
  runtime/                      # 运行视图·工作区汇总
    data-flow.md                #   ≈ use-case 编排并集,默认自动生成  (自动生成,勿手写)
    process-topology.md         #   服务/传输拓扑,可选·有料才建       (可选,人工叙事)
  impact/                       # 影响视图:多为自动生成,只存少量人工叙事页
    risk-map.md                 #   已知脆弱点,人判断                 (人工叙事,改动只标 stale)
...
- 跨仓关注点不单独成页：接口归 `contracts/`、风险归 `impact/risk-map`、依赖归 `architecture/dependency-graph`。
- `process-topology` 仅当存在 contracts 的 producer/consumer 无法表达的运行/传输拓扑时才建。
- `glossary` 是术语→链接索引，不存第二份定义。
- `runtime-notes` 在 error-handling 与 gotchas 内容都较薄时合一；任一方内容量大时各自独立成页。
- 仓内不复刻视图文件夹；只有 `modules/`、`flows/` 这类多实例单元才成文件夹。

## 工作区页面的三种维护方式（决定增量刷新行为）
...
| **自动生成**（由工具/查询生成，勿手写） | `_map`、`dependency-graph`、`data-flow`、`tech-stack`、各 catalog | **不手写、不手维护**；由 query/update 工具从 frontmatter + 双链生成，标 `status: generated` |
| **人工叙事**（需人工综合判断） | `system-architecture`、`risk-map`、`shared-patterns`、`process-topology` | 增量时**只打 `status: stale`、不重写**；由 `obsidian-kb-update` 批量刷新 |
...
**不**在每次增量里维护所有地图。自动生成页读时投影；人工叙事页批量刷新——那是 `obsidian-kb-update` 的活。

## 索引页纪律

- **不**默认生成 `indexes/` 这类 thin 索引目录。
- 视图索引/地图应**靠 frontmatter 查询 + 双链自动生成**，或并进 catalog 的 `_map.md`。
- 持久知识住在可读页面里：`use-cases/`、`domains/`、`contracts/`、`repos/`、`architecture/` 是知识的权威来源。
