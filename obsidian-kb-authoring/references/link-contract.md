...
**对 agent 而言，知识库的导航与影响面判断不靠目录，靠双链遍历。** 因此双链的完整性和双向性，比任何目录调整都重要——影响视图的价值 100% 押在这里：缺一条反向链，影响面就静默漏报。
...
- 代价是**必须在 `global/architecture/coverage.md` 悬挂边表挂账**——缺口不许静默，要显式可读，query 才能把它当"已知的未接边"报出来，而不是漏报。
