...
增量铁律：深度分析也**只做加法**——写本流程文件夹、把可复用定义提升到对应的定义页、append `log.md`，**不碰**工作区人工叙事页 `system-architecture`；跨边界对端落在未 ingest 的仓时建 `status: partial` 契约并在 `architecture/coverage.md` 挂账；不全量重建工作区地图。
...
**契约提升（关键）**：可复用的契约定义——消息标识、payload schema、字段定义、producer/consumer、接收方发现证据——一律**提升/同步到 `{kb-root}/contracts/{契约名}.md`**（契约视图的定义页，只新增）。`跨边界数据流.md` **不重抄 schema**，只持有：
...
- 提升到 `contracts/` 的契约、提升到 `data-models` 的结构都加了反向链，关系双向闭环 → 见 references/link-contract。
