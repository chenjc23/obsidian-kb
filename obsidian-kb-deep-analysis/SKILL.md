...
增量铁律：深度分析也**只做加法**——写本流程文件夹、把可复用定义提升到对应的定义页、append `log.md`，**不碰**工作区人工叙事页 `system-architecture`；跨边界对端落在未 ingest 的仓时建 `status: partial` 契约并在 `global/architecture/coverage.md` 挂账；不全量重建工作区地图。
...
只在加双链或更新共享定义页（`global/contracts/{契约名}.md`、`repos/{repo}/data-models.md`、`repos/{repo}/architecture.md`、`log.md`）时编辑本文件夹外的文件。
...
- **完成判据**：文件夹必须含 `调用树.md`、`主干流程.md`、`自查报告.md`；`跨边界数据流.md`/`数据结构.md`/`{分支主题}.md` 按适用性产出，不适用时在 `自查报告.md` 写明原因（不静默省略）。全 phase 跑完才算完成。
...
- **对端落在尚未 ingest 的仓**（不是搜漏，是那仓还没进库）：把契约提升为 `global/contracts/{X}.md` 但只填已知端、`status: partial`，并在 `global/architecture/coverage.md` 悬挂边表挂账（边类型/已知端/缺失端指向哪个仓/证据/关联契约页）。等那仓 ingest 后接合。
...
- 提升到 `global/contracts/` 的契约、提升到 `data-models.md` 的结构都已加反向链；任何 `status: partial` 契约都在 `coverage.md` 悬挂边表有对应挂账行。
...
- 提升到 `global/contracts/` 的契约、提升到 `data-models` 的结构都加了反向链，关系双向闭环 → 见 references/link-contract。
