...
增量铁律：深度分析也**只做加法**——写本流程文件夹、把可复用定义提升到对应的定义页、把新识别的业务域/用例/契约接入视图层、append `log.md`，**不碰**工作区人工叙事页 `system-architecture`；跨边界对端落在未 ingest 的仓时建 `status: partial` 契约并在 `global/architecture/coverage.md` 挂账；不全量重建工作区地图。
...
- 提升到 `global/domains/`、`global/use-cases/`、`global/contracts/` 的视图层页，以及提升到 `data-models.md` 的结构都已加反向链；任何 `status: partial` 契约都在 `coverage.md` 悬挂边表有对应挂账行。
...
- 提升到 `global/domains/`、`global/use-cases/`、`global/contracts/` 的视图层页，以及提升到 `data-models` 的结构都加了反向链，关系双向闭环 → 见 references/link-contract。
